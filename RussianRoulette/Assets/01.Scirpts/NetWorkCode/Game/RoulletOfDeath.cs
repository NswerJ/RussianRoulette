...
public class RoulletOfDeath : MonoBehaviour
{
    ...
    public Turn turn;
    public ChamberState[] chambers;    // 러시안 룰렛 총실 배열.
    ...
    void ResetGame()
    {
        ...
        _turnManager.BulletSetting(chambers);
    }
    ...
    void Update()
    {
        switch (progress)
        {
            ...
            case GameProgress.Turn:
                ...
                if (turn == Turn.Player)
                {
                    _turnManager.GunControllCompo.GunCompo.MyTurnShootMotion();
                    // 플레이어의 턴일 때의 처리.
                    PlayerTurn();
                    Debug.Log("플레이어 턴");
                }
                ...
                {
                    _turnManager.GunControllCompo.GunCompo.OtherTurnShootMotion();
                    // 상대의 턴일 때의 처리.
                    OpponentTurn();
                    Debug.Log("적 턴");
                }
    ...
    void PlayerTurn()
    {
        ...
        // 총알이 발사되었는지 확인.
        if (chambers[_turnManager.CurrentTurn] == ChamberState.Bullet)
        {
            // 총알이 발사되었을 때의 처리.
            result = Result.Lose;
    ...
    void PullTrigger()
    {
        ...
        turn = _turnManager.GunControllCompo.ShootPlayer(turn);
        chambers[_turnManager.CurrentTurn] = ChamberState.Empty;
